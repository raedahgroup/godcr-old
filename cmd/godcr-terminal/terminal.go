...
func main() {
	...
	// check if user passed commands/options/args in non-cli (terminal) mode
	if len(args) > 0 {
	...
	err = terminal.StartTerminalApp(ctx, walletMiddleware, appConfig.Settings)
	if err != nil {
		exitCode = 1
	}

	// if execution gets to this point, then the terminal app has been quit by the user
	beginShutdown <- true
