...
func main() {
	...
	// check if user passed commands/options/args in non-cli (web) mode
	if len(args) > 0 {
	...
	err = web.StartServer(ctx, walletMiddleware, appConfig.HTTPHost, appConfig.HTTPPort, &appConfig.Settings)
