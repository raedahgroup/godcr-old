...
func PurchaseTicketPage(wallet walletcore.Wallet, setFocus func(p tview.Primitive) *tview.Application, clearFocus func()) tview.Primitive {
	...
	for index, account := range accounts {
		...
		body.AddItem(form.AddDropDown("Source Account", []string{accountOverviews[index]}, 0, func(option string, optionIndex int) {
			...
			AddButton("Submit", func() {
				if len(numTickets) == 0 {
					body.AddItem(textView.SetTextAlign(tview.AlignCenter).SetText(fmt.Sprintf("Error: %s", "Please specify the number of tickets to purchase")).SetDoneFunc(func(key tcell.Key) {
				...
				if len(passphrase) == 0 {
					body.AddItem(textView.SetTextAlign(tview.AlignCenter).SetText(fmt.Sprintf("Error: %s", "please enter your wallet passphrase")).SetDoneFunc(func(key tcell.Key) {
						if key == tcell.KeyEscape {
							clearFocus()
						}
					}), 0, 1, true)
				...
				if err != nil {
					body.AddItem(textView.SetTextAlign(tview.AlignCenter).SetText(fmt.Sprintf("Error: %s", err.Error())), 0, 1, true)
					return
				}
				body.AddItem(textView.SetTextAlign(tview.AlignCenter).SetText(fmt.Sprintf("You have purchased %d ticket(s)\n%s", len(ticketHashes), strings.Join(ticketHashes, "\n"))), 0, 1, true)
			}).SetCancelFunc(func() {
			clearFocus()
		}), 12, 1, true)
	}
